...
#include "usb_device_cdc2.h"
    ...
    #define IN_DATA_BUFFER_ADDRESS_TAG1
    #define OUT_DATA_BUFFER_ADDRESS_TAG1
    #define CONTROL_BUFFER_ADDRESS_TAG1

    #define IN_DATA_BUFFER_ADDRESS_TAG2
    #define OUT_DATA_BUFFER_ADDRESS_TAG2
    #define CONTROL_BUFFER_ADDRESS_TAG2
#endif

//#if !defined(IN_DATA_BUFFER_ADDRESS_TAG1) || !defined(OUT_DATA_BUFFER_ADDRESS_TAG1) || !defined(CONTROL_BUFFER_ADDRESS_TAG1) || !defined(IN_DATA_BUFFER_ADDRESS_TAG2) || !defined(OUT_DATA_BUFFER_ADDRESS_TAG2) || !defined(CONTROL_BUFFER_ADDRESS_TAG2)
...
/** V A R I A B L E S ********************************************************/
volatile unsigned char cdc_data_tx1[CDC_DATA_IN_EP_SIZE] IN_DATA_BUFFER_ADDRESS_TAG1;
volatile unsigned char cdc_data_rx1[CDC_DATA_OUT_EP_SIZE] OUT_DATA_BUFFER_ADDRESS_TAG1;

volatile unsigned char cdc_data_tx2[CDC_DATA_IN_EP_SIZE] IN_DATA_BUFFER_ADDRESS_TAG2;
volatile unsigned char cdc_data_rx2[CDC_DATA_OUT_EP_SIZE] OUT_DATA_BUFFER_ADDRESS_TAG2;
...
LINE_CODING line_coding1, line_coding2;    // Buffer to store line coding information
CDC_NOTICE cdc_notice1, cdc_notice2;

#if defined(USB_CDC_SUPPORT_DSR_REPORTING)
    SERIAL_STATE_NOTIFICATION SerialStatePacket1, SerialStatePacket2;
#endif

uint8_t cdc_rx_len1;            // total rx length
uint8_t cdc_trf_state1;         // States are defined cdc.h
POINTER pCDCSrc1;            // Dedicated source pointer
POINTER pCDCDst1;            // Dedicated destination pointer
uint8_t cdc_tx_len1;            // total tx length
uint8_t cdc_mem_type1;          // _ROM, _RAM

uint8_t cdc_rx_len2;            // total rx length
uint8_t cdc_trf_state2;         // States are defined cdc.h
POINTER pCDCSrc2;            // Dedicated source pointer
POINTER pCDCDst2;            // Dedicated destination pointer
uint8_t cdc_tx_len2;            // total tx length
uint8_t cdc_mem_type2;          // _ROM, _RAM


USB_HANDLE CDCDataOutHandle1;
USB_HANDLE CDCDataInHandle1;

USB_HANDLE CDCDataOutHandle2;
USB_HANDLE CDCDataInHandle2;


CONTROL_SIGNAL_BITMAP control_signal_bitmap1, control_signal_bitmap2;
uint32_t BaudRateGen1, BaudRateGen2;			// BRG value calculated from baudrate

#if defined(USB_CDC_SUPPORT_DSR_REPORTING)
    BM_SERIAL_STATE SerialStateBitmap1, SerialStateBitmap2;
    BM_SERIAL_STATE OldSerialStateBitmap1, OldSerialStateBitmap2;
    USB_HANDLE CDCNotificationInHandle1, CDCNotificationInHandle2;
#endif
...
uint8_t dummy_encapsulated_cmd_response1[dummy_length], dummy_encapsulated_cmd_response2[dummy_length];
...
void USBCheckCDCRequest(void)
{
     ...
     */
    if((SetupPkt.bIntfID != CDC_COMM_INTF_ID1)&&
       (SetupPkt.bIntfID != CDC_DATA_INTF_ID1) && 
       (SetupPkt.bIntfID != CDC_COMM_INTF_ID2) && 
       (SetupPkt.bIntfID != CDC_DATA_INTF_ID2)) return;
    ...
    switch(SetupPkt.bRequest)
    {
        ...
        case SEND_ENCAPSULATED_COMMAND:
         //send the packet
            inPipes[0].pSrc.bRam = (uint8_t*)&dummy_encapsulated_cmd_response1;
        ...
        case SET_LINE_CODING:
            ...
            if((SetupPkt.bIntfID == CDC_COMM_INTF_ID1) || (SetupPkt.bIntfID == CDC_DATA_INTF_ID1))
            {
                outPipes[0].pDst.bRam = (uint8_t*)LINE_CODING_TARGET1;
                outPipes[0].pFunc = LINE_CODING_PFUNC1;
            }
            else if ((SetupPkt.bIntfID == CDC_COMM_INTF_ID2) || (SetupPkt.bIntfID == CDC_DATA_INTF_ID2))
            {
                outPipes[0].pDst.bRam = (uint8_t*)LINE_CODING_TARGET2;
        ...
        case GET_LINE_CODING:
            ...
            {
                USBEP0SendRAMPtr(
                    (uint8_t*)&line_coding1,
                    LINE_CODING_LENGTH,
                    USB_EP0_INCLUDE_ZERO);
            }
            if((SetupPkt.bIntfID == CDC_COMM_INTF_ID2) || (SetupPkt.bIntfID == CDC_DATA_INTF_ID2))
            {
                USBEP0SendRAMPtr(
                    (uint8_t*)&line_coding2,
                    LINE_CODING_LENGTH,
                    USB_EP0_INCLUDE_ZERO);
            }
        ...
        case SET_CONTROL_LINE_STATE:
            if((SetupPkt.bIntfID == CDC_COMM_INTF_ID1) || (SetupPkt.bIntfID == CDC_DATA_INTF_ID1))
                control_signal_bitmap1._byte = (uint8_t)SetupPkt.wValue;
            if((SetupPkt.bIntfID == CDC_COMM_INTF_ID2) || (SetupPkt.bIntfID == CDC_DATA_INTF_ID2))
                control_signal_bitmap2._byte = (uint8_t)SetupPkt.wValue;
            //------------------------------------------------------------------            
            ...
            //------------------------------------------------------------------            
                ...
                if(control_signal_bitmap1.DTE_PRESENT == 1)
                {
...
void CDCInitEP(void)
{
    ...
    line_coding1.dwDTERate   = 19200;      // baud rate
    line_coding1.bCharFormat = 0x00;             // 1 stop bit
    line_coding1.bParityType = 0x00;             // None
    line_coding1.bDataBits = 0x08;               // 5,6,7,8, or 16

    line_coding2.dwDTERate   = 19200;      // baud rate
    line_coding2.bCharFormat = 0x00;             // 1 stop bit
    line_coding2.bParityType = 0x00;             // None
    line_coding2.bDataBits = 0x08;               // 5,6,7,8, or 16
     ...
     *          sent.
     */
    USBEnableEndpoint(CDC_COMM_EP1,USB_IN_ENABLED|USB_HANDSHAKE_ENABLED|USB_DISALLOW_SETUP);
    USBEnableEndpoint(CDC_DATA_EP1,USB_IN_ENABLED|USB_OUT_ENABLED|USB_HANDSHAKE_ENABLED|USB_DISALLOW_SETUP);

    USBEnableEndpoint(CDC_COMM_EP2,USB_IN_ENABLED|USB_HANDSHAKE_ENABLED|USB_DISALLOW_SETUP);
    USBEnableEndpoint(CDC_DATA_EP2,USB_IN_ENABLED|USB_OUT_ENABLED|USB_HANDSHAKE_ENABLED|USB_DISALLOW_SETUP);

    CDCDataOutHandle1 = USBRxOnePacket(CDC_DATA_EP1,(uint8_t*)&cdc_data_rx1,sizeof(cdc_data_rx1));
    CDCDataInHandle1 = NULL;

    CDCDataOutHandle2 = USBRxOnePacket(CDC_DATA_EP2,(uint8_t*)&cdc_data_rx2,sizeof(cdc_data_rx2));
    CDCDataInHandle2 = NULL;
    
    #if defined(USB_CDC_SUPPORT_DSR_REPORTING)
      	CDCNotificationInHandle1 = NULL;
        mInitDTSPin();  //Configure DTS as a digital input
      	SerialStateBitmap1.byte = 0x00;
      	OldSerialStateBitmap1.byte = !SerialStateBitmap1.byte;    //To force firmware to send an initial serial state packet to the host.
        //Prepare a SerialState notification element packet (contains info like DSR state)
        SerialStatePacket1.bmRequestType = 0xA1; //Always 0xA1 for this type of packet.
        SerialStatePacket1.bNotification = SERIAL_STATE;
        SerialStatePacket1.wValue = 0x0000;  //Always 0x0000 for this type of packet
        SerialStatePacket1.wIndex = CDC_COMM_INTF_ID;  //Interface number  
        SerialStatePacket1.SerialState.byte = 0x00;
        SerialStatePacket1.Reserved = 0x00;
        SerialStatePacket1.wLength = 0x02;   //Always 2 bytes for this type of packet    
    ...
    cdc_trf_state1 = CDC_TX_READY;
    cdc_trf_state2 = CDC_TX_READY;
}//end CDCInitEP
 ...
 * 
  ...
  **************************************************************************/
// !!! DRS only used on CDC IF = 0. So function checks only variables for IF = 0 (with 1)
...
void CDCNotificationHandler(void)
{
    ...
    {
        SerialStateBitmap1.bits.DSR = 1;
    }  
    else
    {
        SerialStateBitmap1.bits.DSR = 0;
    }        
    
    //If the state has changed, and the endpoint is available, send a packet to
    //notify the hUSB host of the change.
    if((SerialStateBitmap1.byte != OldSerialStateBitmap1.byte) && (!USBHandleBusy(CDCNotificationInHandle1)))
    {
        //Copy the updated value into the USB packet buffer to send.
        SerialStatePacket1.SerialState.byte = SerialStateBitmap1.byte;
        ...
        CDCNotificationInHandle1 = USBTransferOnePacket(CDC_COMM_EP1, IN_TO_HOST, (uint8_t*)&SerialStatePacket1, sizeof(SERIAL_STATE_NOTIFICATION));
        
        //Save the old value, so we can detect changes later.
        OldSerialStateBitmap1.byte = SerialStateBitmap1.byte;
    }    
}//void CDCNotificationHandler(void)    
#else
...
bool USBCDCEventHandler(USB_EVENT event, void *pdata, uint16_t size)
{
    ...
    {  
        case EVENT_TRANSFER_TERMINATED:
            if(pdata == CDCDataOutHandle1)
            {
                CDCDataOutHandle1 = USBRxOnePacket(CDC_DATA_EP1,(uint8_t*)&cdc_data_rx1,sizeof(cdc_data_rx1));
            }
            if(pdata == CDCDataOutHandle2)
            {
                CDCDataOutHandle2 = USBRxOnePacket(CDC_DATA_EP2,(uint8_t*)&cdc_data_rx2,sizeof(cdc_data_rx2));
            }
            if(pdata == CDCDataInHandle1)
            {
                //flush all of the data in the CDC buffer
                cdc_trf_state1 = CDC_TX_READY;
                cdc_tx_len1 = 0;
            }
            if(pdata == CDCDataInHandle2)
            {
                //flush all of the data in the CDC buffer
                cdc_trf_state2 = CDC_TX_READY;
                cdc_tx_len2 = 0;
...
/**********************************************************************************
  ...
  **********************************************************************************/
uint8_t getsUSBUSART1(uint8_t *buffer, uint8_t len)
{
    cdc_rx_len1 = 0;
    
    if(!USBHandleBusy(CDCDataOutHandle1))
    {
        /*
         * Adjust the expected number of BYTEs to equal
         * the actual number of BYTEs received.
         */
        if(len > USBHandleGetLength(CDCDataOutHandle1))
            len = USBHandleGetLength(CDCDataOutHandle1);
        
        /*
         * Copy data from dual-ram buffer to user's buffer
         */
        for(cdc_rx_len1 = 0; cdc_rx_len1 < len; cdc_rx_len1++)
            buffer[cdc_rx_len1] = cdc_data_rx1[cdc_rx_len1];
...
uint8_t getsUSBUSART2(uint8_t *buffer, uint8_t len)
{
    cdc_rx_len2 = 0;
    
    if(!USBHandleBusy(CDCDataOutHandle2))
    {
        /*
         * Adjust the expected number of BYTEs to equal
         * the actual number of BYTEs received.
         */
        if(len > USBHandleGetLength(CDCDataOutHandle2))
            len = USBHandleGetLength(CDCDataOutHandle2);
        
        /*
         * Copy data from dual-ram buffer to user's buffer
         */
        for(cdc_rx_len2 = 0; cdc_rx_len2 < len; cdc_rx_len2++)
            buffer[cdc_rx_len2] = cdc_data_rx2[cdc_rx_len2];
 ...
 *****************************************************************************/
void putUSBUSART1(uint8_t *data, uint8_t  length)
{
    /*
     ...
     */
    USBMaskInterrupts();
    if(cdc_trf_state1 == CDC_TX_READY)
    {
        mUSBUSARTTxRam1((uint8_t*)data, length);     // See cdc.h
    }
    USBUnmaskInterrupts();
}//end putUSBUSART

void putUSBUSART2(uint8_t *data, uint8_t  length)
{
    /*
     * User should have checked that cdc_trf_state is in CDC_TX_READY state
     * before calling this function.
     ...
     */
    USBMaskInterrupts();
    if(cdc_trf_state2 == CDC_TX_READY)
    {
        mUSBUSARTTxRam2((uint8_t*)data, length);     // See cdc.h
    }
    USBUnmaskInterrupts();
}//end putUSBUSART
...
void putsUSBUSART1(char *data)
{
     ...
     */
    mUSBUSARTTxRam1((uint8_t*)data, len);     // See cdc.h
    USBUnmaskInterrupts();
}//end putsUSBUSART

void putsUSBUSART2(char *data)
{
    uint8_t len;
    char *pData;
     ...
     * which should be called once per Main Program loop.
     */
    mUSBUSARTTxRam2((uint8_t*)data, len);     // See cdc.h
    USBUnmaskInterrupts();
}//end putsUSBUSART
...
/**************************************************************************
  ...
  **************************************************************************/
void putrsUSBUSART1(const const char *data)
{
    uint8_t len;
    ...
    mUSBUSARTTxRom1((const uint8_t*)data,len); // See cdc.h
    USBUnmaskInterrupts();

}//end putrsUSBUSART

void putrsUSBUSART2(const const char *data)
{
    uint8_t len;
    const const char *pData;
    ...
    mUSBUSARTTxRom2((const uint8_t*)data,len); // See cdc.h
    USBUnmaskInterrupts();
...
void CDCTxService1(void)
{
    ...
    if(USBHandleBusy(CDCDataInHandle1)) 
    {
        USBUnmaskInterrupts();
        return;
    }
     ...
     */
    if(cdc_trf_state1 == CDC_TX_COMPLETING)
        cdc_trf_state1 = CDC_TX_READY;
     ...
     */
    if(cdc_trf_state1 == CDC_TX_BUSY_ZLP)
    {
        CDCDataInHandle1 = USBTxOnePacket(CDC_DATA_EP1,NULL,0);
        //CDC_DATA_BD_IN.CNT = 0;
        cdc_trf_state1 = CDC_TX_COMPLETING;
    }
    else if(cdc_trf_state1 == CDC_TX_BUSY)
    {
        /*
         * First, have to figure out how many byte of data to send.
         */
    	if(cdc_tx_len1 > sizeof(cdc_data_tx1))
    	    byte_to_send = sizeof(cdc_data_tx1);
    	else
    	    byte_to_send = cdc_tx_len1;

        /*
         * Subtract the number of bytes just about to be sent from the total.
         */
    	cdc_tx_len1 = cdc_tx_len1 - byte_to_send;
    	  
        pCDCDst1.bRam = (uint8_t*)&cdc_data_tx1; // Set destination pointer
        
        i = byte_to_send;
        if(cdc_mem_type1 == USB_EP0_ROM)            // Determine type of memory source
        {
            while(i)
            {
                *pCDCDst1.bRam = *pCDCSrc1.bRom;
                pCDCDst1.bRam++;
                pCDCSrc1.bRom++;
                i--;
        ...
        if(cdc_tx_len1 == 0)
        {
            if(byte_to_send == CDC_DATA_IN_EP_SIZE)
                cdc_trf_state1 = CDC_TX_BUSY_ZLP;
            else
                cdc_trf_state1 = CDC_TX_COMPLETING;
        }//end if(cdc_tx_len...)
        CDCDataInHandle1 = USBTxOnePacket(CDC_DATA_EP1,(uint8_t*)&cdc_data_tx1,byte_to_send);
...
void CDCTxService2(void)
{
    ...
    //CDCNotificationHandler(); // DRS is NOT used on second CDC
    
    if(USBHandleBusy(CDCDataInHandle2)) 
    {
        USBUnmaskInterrupts();
        return;
    }
     ...
     * and not having to call mCDCUsartTxIsBusy() directly.
     */
    if(cdc_trf_state2 == CDC_TX_COMPLETING)
        cdc_trf_state2 = CDC_TX_READY;
     ...
     * If CDC_TX_BUSY_ZLP state, send zero length packet
     */
    if(cdc_trf_state2 == CDC_TX_BUSY_ZLP)
    {
        CDCDataInHandle2 = USBTxOnePacket(CDC_DATA_EP2,NULL,0);
        //CDC_DATA_BD_IN.CNT = 0;
        cdc_trf_state2 = CDC_TX_COMPLETING;
    }
    else if(cdc_trf_state2 == CDC_TX_BUSY)
    {
        /*
         * First, have to figure out how many byte of data to send.
         */
    	if(cdc_tx_len2 > sizeof(cdc_data_tx2))
    	    byte_to_send = sizeof(cdc_data_tx2);
    	else
    	    byte_to_send = cdc_tx_len2;

        /*
         * Subtract the number of bytes just about to be sent from the total.
         */
    	cdc_tx_len2 = cdc_tx_len2 - byte_to_send;
    	  
        pCDCDst2.bRam = (uint8_t*)&cdc_data_tx2; // Set destination pointer
        
        i = byte_to_send;
        if(cdc_mem_type2 == USB_EP0_ROM)            // Determine type of memory source
        {
            while(i)
            {
                *pCDCDst2.bRam = *pCDCSrc2.bRom;
                pCDCDst2.bRam++;
                pCDCSrc2.bRom++;
                i--;
            }//end while(byte_to_send)
        ...
        {
            if(byte_to_send == CDC_DATA_IN_EP_SIZE)
                cdc_trf_state2 = CDC_TX_BUSY_ZLP;
            else
                cdc_trf_state2 = CDC_TX_COMPLETING;
        }//end if(cdc_tx_len...)
        CDCDataInHandle2 = USBTxOnePacket(CDC_DATA_EP2,(uint8_t*)&cdc_data_tx2,byte_to_send);
